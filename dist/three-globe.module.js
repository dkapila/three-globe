...
function createGlowMaterial(coefficient, color, power) {
  return new THREE$6.ShaderMaterial({
    depthWrite: false,
...
function createGlowGeometry(geometry, size) {
  ...
  glowGeometry.setAttribute('position', new THREE$6.BufferAttribute(position, 3));
  return glowGeometry;
}
function createGlowMesh(geometry) {
  ...
  if (backside) {
    glowMaterial.side = THREE$6.BackSide;
  }

  return new THREE$6.Mesh(glowGeometry, glowMaterial);
}
...
var GlobeLayerKapsule = Kapsule({
  ...
  stateInit: function stateInit() {
    // create globe
    var globeGeometry = new THREE$5.SphereBufferGeometry(GLOBE_RADIUS, 75, 75);
    var defaultGlobeMaterial = new THREE$5.MeshPhongMaterial({
      color: 0x000000,
      transparent: true
    });
    var globeObj = new THREE$5.Mesh(globeGeometry, defaultGlobeMaterial);
    globeObj.rotation.y = -Math.PI / 2; // face prime meridian along Z axis

    globeObj.__globeObjType = 'globe'; // Add object type
    // create graticules

    var graticulesObj = new THREE$5.LineSegments(new GeoJsonGeometry(geoGraticule10(), GLOBE_RADIUS, 2), new THREE$5.LineBasicMaterial({
      color: 'lightgrey',
      transparent: true,
      opacity: 0.1
    }));
  ...
  update: function update(state, changedProps) {
    ...
    if (changedProps.hasOwnProperty('globeImageUrl')) {
      if (!state.globeImageUrl) {
        ...
        !globeMaterial.color && (globeMaterial.color = new THREE$5.Color(0x000000));
      } else {
        new THREE$5.TextureLoader().load(state.globeImageUrl, function (texture) {
    ...
    if (changedProps.hasOwnProperty('bumpImageUrl')) {
      if (!state.bumpImageUrl) {
        ...
        state.bumpImageUrl && new THREE$5.TextureLoader().load(state.bumpImageUrl, function (texture) {
...
var applyMatrix4Fn = new THREE$4.BufferGeometry().applyMatrix4 ? 'applyMatrix4' : 'applyMatrix';
var PointsLayerKapsule = Kapsule({
  ...
  update: function update(state) {
    ...
    var pointGeometry = new THREE$4.CylinderBufferGeometry(1, 1, 1, state.pointResolution);
    pointGeometry[applyMatrix4Fn](new THREE$4.Matrix4().makeRotationX(Math.PI / 2));
    pointGeometry[applyMatrix4Fn](new THREE$4.Matrix4().makeTranslation(0, 0, -0.5));
    var pxPerDeg = 2 * Math.PI * GLOBE_RADIUS / 360;
    var pointMaterials = {}; // indexed by color

    var scene = state.pointsMerge ? new THREE$4.Object3D() : state.scene; // use fake scene if merging points
    ...
    if (state.pointsMerge) {
      ...
      var pointsGeometry = !state.pointsData.length ? new THREE$4.BufferGeometry() : BufferGeometryUtils.mergeBufferGeometries(state.pointsData.map(function (d) {
        ...
        geom.setAttribute('color', new THREE$4.BufferAttribute(colors, 3));
        return geom;
      }));
      var points = new THREE$4.Mesh(pointsGeometry, new THREE$4.MeshBasicMaterial({
        color: 0xffffff,
        vertexColors: true,
        morphTargets: false
      }));
    ...
    function createObj() {
      var obj = new THREE$4.Mesh(pointGeometry);
    ...
    function updateObj(obj, d) {
      var applyUpdate = function applyUpdate(td) {
        ...
        var globeCenter = state.pointsMerge ? new THREE$4.Vector3(0, 0, 0) : state.scene.localToWorld(new THREE$4.Vector3(0, 0, 0)); // translate from local to world coords
      ...
      if (!state.pointsMerge) {
        ...
        if (!pointMaterials.hasOwnProperty(color)) {
          pointMaterials[color] = new THREE$4.MeshLambertMaterial({
...
var linkedGlobeLayerProps = Object.assign.apply(Object, _toConsumableArray(['globeImageUrl', 'bumpImageUrl', 'showGlobe', 'showAtmosphere', 'atmosphereColor', 'atmosphereAltitude'].map(function (p) {
